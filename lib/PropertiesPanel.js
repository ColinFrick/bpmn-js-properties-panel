...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  if (bowser.msie) { // Internet Explorer doesn't support input events on contenteditable elements
    domDelegate.bind(container, 'input, textarea', 'input', debounce(handleChange, DEBOUNCE_DELAY));
    domDelegate.bind(container, '[contenteditable]', 'DOMCharacterDataModified', debounce(handleChange, DEBOUNCE_DELAY));
    domDelegate.bind(container, '[contenteditable]', 'DOMNodeRemoved', debounce(handleChange, DEBOUNCE_DELAY));
  } else {
...
PropertiesPanel.prototype.updateState = function(entry, entryNode) {
  ...
  this.updatePlaceholder(entry, entryNode);
};
...
/**
 * Evaluates a given function. The return value is then set as the placeholder attribute
 */
PropertiesPanel.prototype.updatePlaceholder = function(entry, node) {
  var current = this._current;
  ...
  var nodes = domQuery.all('[data-placeholder]', node) || [];

  forEach(nodes, function(currentNode) {
    var expr = domAttr(currentNode, 'data-placeholder');
    var fn = get(entry, expr);
    if (fn) {
      var scope = domClosest(currentNode, '[data-scope]') || node;
      var placeholder = fn(current.element, node, currentNode, scope);

      domAttr(currentNode, 'placeholder', placeholder || null);
    }
  });
};
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  function updateLabel(element, selector, text) {
    ...
    // Check if text is a DOM element replace content with it
    if (isElement(text)) {
      labelNode.innerHTML = '';
  ...
  forEach(current.tabs, function(tab) {
    ...
    if (typeof tab.label === 'function') {
      updateLabel(tabLinkNode, '.tab-label', tab.label(element, tabNode));
    }
  ...
  forEach(tabs, function(tab, tabIndex) {
    ...
    var tabNode = domify('<div class="bpp-properties-tab" data-tab="' + tab.id + '"></div>'),
        tabLinkNode = domify('<li class="bpp-properties-tab-link">' +
          '<a href class="tab-label" data-tab-target="' + tab.id + '">' + tab.label + '</a>' +
        '</li>');
